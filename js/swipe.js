...
//                 break;
    ...
    el[0].addEventListener(events.start, checkStart);
    el[0].addEventListener(events.move, checkMove);
    el[0].addEventListener(events.end, checkEnd);
...
var examples = {
    Up: {
        el: getExampleDiv,
        callback: function (e) {
            makeDone(e.target, e.detail.dir, "up");
        }
    },
    Down: {
        el: getExampleDiv,
...
for (var example in examples) {
    ...
    el[0].addEventListener("swipe", examples[example].callback);
}
